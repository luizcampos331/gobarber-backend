...
let ResetPasswordService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const userToken = await this.userTokensRepository.findByToken(token); // Check if token exists

    if (!userToken) {
      throw new _AppError.default('User token does not exists');
    }

    const user = await this.usersRepository.findById(userToken.user_id); // Check if user exists
    ...
    const compareDate = (0, _dateFns.addHours)(tokenCreatedAt, 2); // Check if the creation of the token was more than 2 hours
