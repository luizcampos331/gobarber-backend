...
let UpdateUserAvatarService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const user = await this.usersRepository.findById(user_id); // === Check User
    ...
    const filename = await this.storageProvider.saveFile(avatarFilename); // === Update avatar User

    user.avatar = filename; // === Save update on database
