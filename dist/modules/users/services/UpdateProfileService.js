...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const user = await this.usersRepository.findById(user_id); // Check if user exists
    ...
    user.name = name;
    user.email = email; // Check if old password or new password not were inform
    ...
    if (password && old_password) {
      ...
      const checkOldPassword = await this.hashProvider.compareHase(old_password, user.password);
