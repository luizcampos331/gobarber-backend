...
let SendForgotPasswordEmailService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const user = await this.usersRepository.findByEmail(email); // Check if user exists
    ...
    const forgotTemplatePassword = _path.default.resolve(__dirname, '..', 'views', 'forgot_password.hbs');
